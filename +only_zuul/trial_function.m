...
switch state
    ...
    case p.trial.pldaps.trialStates.trialSetup
        ...
        fprintf('\tThis is a %s %s trial with log contrast %0.3f.\n',upper(p.trial.task.condition.symbol_type),upper(p.trial.task.condition.trial_type),p.trial.task.condition.log10C);
        if(p.trial.task.training.repeat_priority>=p.trial.task.condition.repeat_priority)
            fprintf('\tThis trial must be repeated until completed correctly.\n');
        end
    ...
    case p.trial.pldaps.trialStates.trialCleanUpandSave
        ...
        %
        %  Repeat the trial if it has an appropriate repeat priority.
        %  Otherwise update the trial index.
        if(p.trial.task.outcome.completed)
            if(p.trial.task.training.repeat_priority<p.trial.task.condition.repeat_priority || p.trial.task.outcome.correct)
            ...
            p.trial.task.performance.matrix(p.trial.task.condition.lum_indx,1) = p.trial.task.performance.matrix(p.trial.task.condition.lum_indx,1)+((p.trial.task.outcome.correct && p.trial.task.state_variables.release_trial) || (~p.trial.task.outcome.correct && p.trial.task.state_variables.press_trial));
            p.trial.task.performance.matrix(p.trial.task.condition.lum_indx,2) = p.trial.task.performance.matrix(p.trial.task.condition.lum_indx,2)+((p.trial.task.outcome.correct && p.trial.task.state_variables.press_trial) || (~p.trial.task.outcome.correct && p.trial.task.state_variables.release_trial));
            p.trial.task.performance.matrix(p.trial.task.condition.lum_indx,3) = p.trial.task.performance.matrix(p.trial.task.condition.lum_indx,3)+1;
        else
            p.trial.task.indexing.next_trial = p.trial.task.indexing.current_trial;
            p.conditions{end+1} = p.conditions{end};
            ...
            case 'symbol'
                ...
                if(~p.trial.task.training.use_symbol_masks || p.trial.task.timing.symbol.duration==0)
                    ...
                    if(isnan(p.trial.task.timing.symbol.start_time))
                        ...
                        if(strcmp('mask',p.trial.task.condition.symbol_type)==1)
                            ShowSymbolMask;
                        else
                            ShowSymbol;
                        end
                        fprintf('SYMBOL %s %d of 3 for %0.3f sec.\n',p.trial.task.condition.symbol_type,p.trial.task.state_variables.current_symbol,p.trial.task.timing.symbol.duration);
                ...
                %                 switch p.trial.task.condition.trial_type
                %                     case {'set','notset'}
