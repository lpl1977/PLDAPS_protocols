classdef indexing < handle
    ...
    methods
        %  Class constructor
        function obj = indexing(shuffle_groups)
            
            obj.num_trials = length(shuffle_groups);
            
            obj.shuffle_groups = shuffle_groups;
            obj.trial_list = 1:obj.num_trials;
            
            %  Shuffle trial list within blocks
            ublocks = unique(shuffle_groups);
            for i=1:length(ublocks)
                indx = shuffle_groups==ublocks(i);
        ...
        function ix = current_block(obj)
            ix = obj.shuffle_groups(obj.current_trial);
        end
        ...
        function [first_trial,last_trial] = shuffle(obj)
            ...
            indx = obj.shuffle_groups==obj.current_block;
