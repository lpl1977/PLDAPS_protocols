...
%  Control flags
%
p.functionHandles.controlFlags.useRewardIndicator = true;       %  Use the secondary reinforcer 
p.functionHandles.controlFlags.useRandomDisplacement = false;
p.functionHandles.controlFlags.repeatErrorTrialLikelihood = 0;
p.functionHandles.controlFlags.useInRegionReward = true;
p.functionHandles.controlFlags.useReturnReward = true;
p.functionHandles.controlFlags.useOvershootPenalty = false;
p.functionHandles.controlFlags.useInterSymbolInterval = true;
p.functionHandles.controlFlags.useCenterRewardRegion = true;
p.functionHandles.controlFlags.useCenterSelectionTimer = false;
p.functionHandles.controlFlags.useRewardRegions = true;
p.functionHandles.controlFlags.useSelectionColorChange = true;

%  Reward
if(p.functionHandles.controlFlags.useInRegionReward)
    p.functionHandles.reward.inRegion = 0.1;
else
    p.functionHandles.reward.inRegion = 0;
end
if(p.functionHandles.controlFlags.useReturnReward)
    p.functionHandles.reward.atReturn = 0.3;
else
    p.functionHandles.reward.atReturn = 0;
end
p.functionHandles.reward.maxDuration = p.functionHandles.reward.inRegion + p.functionHandles.reward.atReturn;

%  Timing
p.functionHandles.timing.postRewardDelay = max(0.5,p.functionHandles.reward.atReturn);
...
%  Geometry of stimuli
p.functionHandles.geometry.symbolDisplacement = 525;
p.functionHandles.geometry.symbolRadius = 150;
p.functionHandles.geometry.rewardRegionBuffer = 50;
p.functionHandles.geometry.center = [959.5 539.5];
temp = p.functionHandles.geometry.symbolDisplacement+p.functionHandles.geometry.symbolRadius+p.functionHandles.geometry.rewardRegionBuffer;
p.functionHandles.geometry.extent = [2*temp 0];
...
%p.functionHandles.drawingFunctions = dmf.drawingFunctions(p.functionHandles.features,patterns,shapes);
%p.functionHandles.drawingFunctions = dmf.drawingFunctions(p.trial.display.ptr,p.functionHandles.features);
